...
# ----------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def line_intersection(self, line1, line2):
        ...
        line_length = math.sqrt(xdiff[1]**2 + ydiff[1]**2)
        if calc_dist(self.Car_Pos, line2[0]) > line_length and calc_dist(self.Car_Pos, line2[1]) > line_length:
            return None
        ...
        big_x = min(max(x1_1, x1_2), max(x2_1, x2_2))
        small_x = max(min(x1_1, x1_2), min(x2_1, x2_2))

        big_y = min(max(y1_1, y1_2), max(y2_1, y2_2))
        small_y = max(min(y1_1, y1_2), min(y2_1, y2_2))
